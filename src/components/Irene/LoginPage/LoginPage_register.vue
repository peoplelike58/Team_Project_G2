...
// 第三方登入(An)
import { useAuthStore } from '@/stores/auth'   // 新增：Pinia Auth store（Google 登入用）
...
// 掛載 Google Identity Services (An)
onMounted(async () => {
...
function initGIS() {
    ...
    if (el) {
        google.accounts.id.renderButton(el, { type: 'standard', size: 'large', theme: 'outline', shape: 'pill' })
    }
}
